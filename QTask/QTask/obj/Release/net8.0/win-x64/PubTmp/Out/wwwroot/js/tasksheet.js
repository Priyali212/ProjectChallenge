...
function ShowJiraId(obj) {
    ...
    if (obj != null) {
        ...
        $(obj).each(function () {
            ...
            $(tdAction).html('<div class="td-flex"><button class="blue-btn slide-action-down" id="idDown"><i class="fa fa-chevron-down"></i></button><button class="blue-btn slide-action-up" id="idUp"><i class="fa fa-chevron-up"></i></button> <button class="blue-btn slide-action-btn"><i class="fa fa-cog"></i></button> </div>');
        ...
        $('#dvJiraList td i.fa-chevron-up, #dvJiraList td i.fa-chevron-down').click(function () {
            rowMove($(this));
        });

        $('#dvJiraList').find('span.pg-no').attr('page-id', PageIndex.toString()).text((((parseInt(PageIndex.toString()) - 1) * 20) + 1).toString());
        $('#dvJiraList').find('span.pg-current-total').text((parseInt(PageIndex) * 20) > parseInt(TotalCount) ? TotalCount.toString() : (parseInt(PageIndex) * 20).toString());
      

        if (parseInt(PageIndex) < 2) {
            $('#dvJiraList').find('button.prev').attr('disabled', 'disabled');
            $('#dvJiraList').find('button.first').attr('disabled', 'disabled');
        }
        else {
            $('#dvJiraList').find('button.prev').removeAttr('disabled');
            $('#dvJiraList').find('button.first').removeAttr('disabled');
        }
       
        if (((parseInt(TotalCount) % 20) == 0 && (parseFloat(TotalCount) / 20 > parseFloat(PageIndex))) || ((parseInt(TotalCount) % 20) != 0 && (parseFloat(PageIndex) / 20 >= parseFloat(PageIndex)))) {
            $('#dvJiraList').find('button.next').removeAttr('disabled');
            $('#dvJiraList').find('button.last').removeAttr('disabled');
        }
        else {
            $('#dvJiraList').find('button.next').attr('disabled', 'disabled');
            $('#dvJiraList').find('button.last').attr('disabled', 'disabled');
        }
...
function ShowTasksheetList() {
    ...
    $.get(url, { JiraID: JiraID, ProjectName: ProjectName, AssignedTo: AssignedTo, Status: Status },
        function (resonse) {
            if (resonse != null) {
